...
def should_continue(state: InterviewState) -> str:
    """
    Determines whether to continue the interview or proceed to final evaluation.

    Args:
        state (InterviewState): Current state containing `step` and `max_questions`.

    Returns:
        str: Next node identifier ("generate_question" or "final_evaluation").
    """
    ...
    logger.debug("Progress: step=%d / max=%d → next=%s", step, max_questions, next_node)
    return next_node


def create_interview_graph() -> StateGraph:
    """
    Builds and compiles the LangGraph-based interview workflow.
    ...
    Returns:
        StateGraph: A fully compiled interview graph.
    """
    ...
    logger.debug("All graph nodes registered.")
    ...
    logger.info("Interview graph successfully constructed and compiled.")
