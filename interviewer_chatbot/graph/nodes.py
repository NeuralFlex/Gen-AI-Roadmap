...
def setup_node(state: InterviewState) -> InterviewState:
    ...
    if question_type not in valid_types:
        logger.warning(
            f"Invalid question_type '{question_type}', defaulting to 'broad_followup'."
        )
    ...
    try:
        ...
        first_question = (
            ...
            or "Tell me about your interest in this topic."
        )
    except Exception as e:
    ...
    logger.info(
        f"Setup complete for topic '{topic}' with question type '{question_type}'."
    )
    return {
        **state,
        "topic": topic,
...
def evaluate_question_node(state: InterviewState) -> InterviewState:
    ...
    try:
        q_feedback_text = gemini_client.generate_content(
        ...
        a_feedback_text = gemini_client.generate_content(
            get_evaluation_prompt(
                kind="answer",
                full_messages=full_messages,
                full_content=full_content,
...
def generate_question_node(state: InterviewState) -> InterviewState:
    ...
    if step > 0 and state.get("questions") and state.get("answers"):
        ...
        try:
            tavily_results = tavily_service.search(
                f"{state.get('topic')} interview context: Q: {last_q} A: {last_a}"
            )
